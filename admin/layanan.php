...
<title>Data Layanan - Admin E-SPEED</title>
...
<nav class="sidebar" id="sidebar">
  ...
  <ul>
    ...
    <li><a href="layanan.php" class="active"><span class="icon">🛠️</span> <span>Layanan</span></a></li>
...
<main class="content">
  ...
  <button class="btn" onclick="openAddModal()">+ Tambah Layanan</button>
    ...
    <table id="serviceTable">
      ...
      <tbody>
        ...
        <tr data-id="<?= $row['id_menu'] ?>">
          ...
          <td><span class="price service-price" data-price="<?= $row['price'] ?>">Rp<?= number_format($row['price'], 0, ',', '.') ?></span></td>
          <td>
            <button class="btn-edit" onclick='openEditModal(<?= json_encode($row) ?>)'>Edit</button>
            <button class="btn-hapus" onclick='openDeleteModal("<?= $row['id_menu'] ?>", "<?= htmlspecialchars($row['name']) ?>")'> Hapus</button>
          </td>
        </tr>
        <?php endwhile; else: ?>
        <tr><td colspan="4" class="empty-state">Belum ada layanan</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
    ...
    <div class="modal-body">
      <form id="addForm">
        <div class="form-group">
          ...
          <input type="text" id="add_name" name="name" required placeholder="Contoh: Cuci Motor">
        </div>
        
        <div class="form-group">
          <label>Harga (Rp) *</label>
          <input type="number" id="add_price" name="price" required placeholder="Contoh: 15000" min="0">
        </div>
      </form>
    </div>
    <div class="modal-footer">
      <button class="btn-modal btn-secondary-modal" onclick="closeModal('addModal')">Batal</button>
      <button class="btn-modal btn-success-modal" onclick="addService()">Simpan</button>
    </div>
  </div>
</div>
...
<div id="editModal" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-footer">
      ...
      <button class="btn-modal btn-primary-modal" onclick="updateService()">Update</button>
    </div>
  </div>
</div>
...
<div id="deleteModal" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-footer">
      ...
      <button class="btn-modal btn-danger-modal" onclick="deleteService()">🗑 Hapus</button>
    </div>
  </div>
</div>
...
function addService() {
  ...
  .then(data => {
    if (data.success) {
      ...
      newRow.setAttribute('data-id', data.data.id_menu);
      newRow.innerHTML = `
        <td>${rowCount + 1}</td>
        <td class="service-name">${data.data.name}</td>
        <td><span class="price service-price" data-price="${data.data.price}">${formatRupiah(data.data.price)}</span></td>
        <td>
          <button class="btn-edit" onclick='openEditModal(${JSON.stringify(data.data)})'>✏ Edit</button>
          <button class="btn-hapus" onclick='openDeleteModal("${data.data.id_menu}", "${data.data.name}")'> Hapus</button>
        </td>
      `;
...
function openEditModal(service) {
  document.getElementById('edit_id').value = service.id_menu;
...
function updateService() {
  const formData = new FormData();
  formData.append('id_menu', document.getElementById('edit_id').value);
  formData.append('name', document.getElementById('edit_name').value);
  formData.append('price', document.getElementById('edit_price').value);
  
  fetch('edit_layanan.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
  .then(data => {
    if (data.success) {
      ...
      const serviceId = document.getElementById('edit_id').value;
      const row = document.querySelector(`tr[data-id="${serviceId}"]`);
      if (row) {
        row.querySelector('.service-name').textContent = document.getElementById('edit_name').value;
        ...
        priceElement.textContent = formatRupiah(newPrice);
      }
    } else {
      showAlert(data.message, 'danger');
    }
...
function deleteService() {
  const formData = new FormData();
  formData.append('id_menu', document.getElementById('delete_id').value);
  
  fetch('hapus_layanan.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
  .then(data => {
    if (data.success) {
      ...
      const serviceId = document.getElementById('delete_id').value;
      const row = document.querySelector(`tr[data-id="${serviceId}"]`);
      if (row) {
        row.remove();
      }
      ...
      if (tbody.querySelectorAll('tr').length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="empty-state">Belum ada layanan</td></tr>';
      } else {
        // Update nomor urut
        updateRowNumbers();
      }
